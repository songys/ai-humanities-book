"""
Beamer Madrid 색상 시스템 다이어그램 16개 생성 스크립트
...
# ============================================================
# Beamer Madrid 컬러 팔레트
# ============================================================
# Primary colors (strong, for borders/text/headers)
NAVY      = '#002060'
GREEN     = '#2E7D32'
RED       = '#C62828'
ORANGE    = '#E65100'
...
def make_axes(figsize, xlim, ylim, title):
    ...
    ax.axis('off')
    ax.set_title(title, fontsize=14, fontweight='bold', color=NAVY, pad=18)
    return fig, ax
...
def diagram_01():
    ...
    eras = [
        (0.5, '2000s', '디지털 아카이브\n전자 텍스트 구축', NAVY_BG, NAVY),
        (3.0, '2010s', '빅데이터 분석\n텍스트 마이닝', PURPLE_BG, PURPLE),
        (5.5, '2020s', '생성형 AI\nLLM 등장', GREEN_BG, GREEN),
        (8.0, '현재', 'AI와 인문학 협업\n바이브 코딩', AMBER_BG, AMBER),
    ]

    for i, (x, era, desc, bg, border) in enumerate(eras):
        circle = plt.Circle((x + 0.5, 1.8), 0.38, color=bg, ec=border, lw=1.5, zorder=3)
        ax.add_patch(circle)
...
def diagram_02():
    ...
    layers = [
        (-4, 0, 8, 1.0, NAVY_BG, NAVY, '인공지능 (AI)', '사람의 지능을 모방하는 모든 기술'),
        (-3, 1.2, 6, 1.0, PURPLE_BG, PURPLE, '머신러닝 (ML)', '데이터에서 패턴을 학습'),
        (-2, 2.4, 4, 1.0, GREEN_BG, GREEN, '딥러닝 (DL)', '인공 신경망으로 복잡한 패턴 학습'),
        (-1, 3.6, 2, 1.0, AMBER_BG, AMBER, 'LLM', '대규모 언어모델'),
    ]

    for x, y, w, h, bg, border, label, desc in layers:
...
def diagram_03():
    ...
    cards = [
        ...
        (5.0, 3, '텍스트 분석', '수천 편의 문학 작품을\n분석하고 패턴을 발견', PURPLE_BG, PURPLE),
        (0.5, 0.2, '번역과 소통', '언어의 장벽을 넘어\n다양한 문화를 탐색', GREEN_BG, GREEN),
        (5.0, 0.2, '시각적 스토리텔링', '텍스트를 이미지로 변환\n새로운 표현 방식 탐구', AMBER_BG, AMBER),
    ]

    for x, y, title, desc, bg, border in cards:
...
def diagram_04():
    ...
    # 검색 AI
    rounded_box(ax, 0, 1.0, 4.5, 2.2, RED_BG, RED)
    ...
    # 생성형 AI
    rounded_box(ax, 6.5, 1.0, 4.5, 2.2, GREEN_BG, GREEN)
    ...
    # 핵심 차이
    rounded_box(ax, 2.5, -0.4, 6.0, 0.85, AMBER_BG, AMBER,
                "핵심: '찾아준다' vs '만들어준다'", fontsize=10, text_color=AMBER)
...
def diagram_05():
    ...
    eras = [
        (0.2, 'Classical ML\n1990~2010s', 'SVM, Random Forest\n정형 데이터 중심', NAVY_BG, NAVY),
        (3.8, '전환점\n2017~2022', 'Transformer 등장\nGPT, ChatGPT', PURPLE_BG, PURPLE),
        (7.4, 'LLM 시대\n2020s~', '텍스트 생성 중심\n멀티모달, 바이브 코딩', GREEN_BG, GREEN),
    ]

    for x, era, desc, bg, border in eras:
...
def diagram_06():
    ...
    # 정형 데이터
    rounded_box(ax, 0.3, 1.0, 3.5, 2.2, NAVY_BG, NAVY)
    ...
    # 비정형 데이터
    rounded_box(ax, 5.5, 1.0, 4.5, 2.2, GREEN_BG, GREEN)
    ...
    # 핵심
    rounded_box(ax, 2.0, -0.3, 6.5, 0.85, AMBER_BG, AMBER,
                '데이터의 종류가 AI의 가능성을 결정한다', fontsize=10, text_color=AMBER)
...
def diagram_07():
    ...
    steps = [
        (0.2, '1단계', '대량의 텍스트\n학습', NAVY_BG, NAVY),
        (2.9, '2단계', '다음 단어\n예측 능력', PURPLE_BG, PURPLE),
        (5.6, '3단계', '사용자 질문에\n답변 생성', GREEN_BG, GREEN),
        (8.3, '4단계', '대화로\n답변 개선', AMBER_BG, AMBER),
    ]

    for x, step, desc, bg, border in steps:
...
def diagram_08():
    ...
    # 오해 1
    rounded_box(ax, 0.3, 3.0, 8.8, 1.6, RED_BG, RED)
    ...
    # 오해 2
    rounded_box(ax, 0.3, 1.0, 8.8, 1.6, RED_BG, RED)
    ...
    # 시사점
    rounded_box(ax, 1.5, -0.4, 6.5, 0.85, GREEN_BG, GREEN,
                '어떤 데이터를 선택하느냐는 가치 판단의 문제', fontsize=10, text_color=GREEN)
...
def diagram_09():
    ...
    principles = [
        ...
        (5.0, 4.2, '02 맥락 제공', '나의 상황과\n배경 정보 전달', PURPLE_BG, PURPLE),
        (0.5, 1.5, '03 역할 부여', 'AI에게\n전문가 역할 부여', GREEN_BG, GREEN),
        (5.0, 1.5, '04 출력 형식', '답변의 형태를\n명시', AMBER_BG, AMBER),
    ]

    for x, y, title, desc, bg, border in principles:
    ...
    # 중앙 레이블
    center = plt.Circle((4.65, 3.6), 0.5, color=NAVY_BG, ec=NAVY, lw=1.5, zorder=5)
    ax.add_patch(center)
...
def diagram_10():
    ...
    # Before
    rounded_box(ax, 0.3, 0.5, 4.3, 2.7, RED_BG, RED)
    ...
    # After
    rounded_box(ax, 5.9, 0.5, 4.3, 2.7, GREEN_BG, GREEN)
...
def diagram_11():
    ...
    elements = [
        (0.3, '화자', NAVY_BG, NAVY),
        (2.3, '독자', PURPLE_BG, PURPLE),
        (4.3, '종결 어미', GREEN_BG, GREEN),
        (6.3, '비유 여부', AMBER_BG, AMBER),
    ]

    for x, label, bg, border in elements:
    ...
    box = FancyBboxPatch((8.6, 1.3), 1.6, 2.0, boxstyle="round,pad=0.15",
                         facecolor=NAVY_BG, edgecolor=NAVY, linewidth=1.5)
    ax.add_patch(box)
...
def diagram_12():
    ...
    # Beamer 색상으로 6가지 말투 표현
    tones = {
        'A. 강의형':   ([5, 3, 2, 3, 2, 3], NAVY),
        'B. 경험자형': ([4, 4, 3, 4, 3, 3], PURPLE),
        'C. 보고서형': ([1, 5, 5, 3, 5, 2], GREEN),
        'D. 선배형':   ([5, 2, 1, 2, 2, 5], AMBER),
        'E. 마케팅형': ([3, 2, 3, 5, 2, 5], RED),
...
def diagram_13():
    ...
    problems = [
        (0.3, 3.2, '① 문장 단절', '짧고 끊긴 문장', RED_BG, RED),
        (3.6, 3.2, '② 문단 분절', '지나친 줄바꿈', RED_BG, RED),
        (6.9, 3.2, '③ 번역투', '영어 직역 표현', RED_BG, RED),
        (0.3, 0.8, '④ 슬롭 워드', 'AI 과용 단어', ORANGE_BG, ORANGE),
        (3.6, 0.8, '⑤ 종결 단조', '~합니다만 반복', ORANGE_BG, ORANGE),
        (6.9, 0.8, '⑥ 메타 해설', '투어 가이드 문장', ORANGE_BG, ORANGE),
    ]

    for x, y, title, desc, bg, border in problems:
...
def diagram_14():
    ...
    steps = [
        (0.5, 3.5, '첫 질문', '일반적인 질문으로 시작', NAVY_BG, NAVY),
        (0.5, 2.1, '범위 좁히기', "'그중에서 ~에 집중해줘'", PURPLE_BG, PURPLE),
        (0.5, 0.7, '구체화', "'예를 들어 설명해줘'", GREEN_BG, GREEN),
    ]

    strategies = [
        (5.5, 3.5, '형식 바꾸기', "'표로 정리해줘'", AMBER_BG, AMBER),
        (5.5, 2.1, '수준 조절', "'쉽게 설명해줘'", ORANGE_BG, ORANGE),
        (5.5, 0.7, '비판 요청', "'문제점을 지적해줘'", NAVY_BG, NAVY),
    ]

    for items in [steps, strategies]:
        for x, y, title, desc, bg, border in items:
...
def diagram_15():
    ...
    limits = [
        ...
        (2.3, '최신 정보\n부족', '학습 데이터 이후\n정보를 모름', ORANGE_BG, ORANGE),
        (4.3, '개인 경험\n부재', '자기만의 경험이나\n감정이 없음', NAVY_BG, NAVY),
        (6.3, '수학적\n추론 한계', '복잡한 계산에서\n실수', PURPLE_BG, PURPLE),
        (8.3, '일관성\n부족', '같은 질문에\n다른 답변', AMBER_BG, AMBER),
    ]

    for x, title, desc, bg, border in limits:
...
def diagram_16():
    ...
    # Part I
    rounded_box(ax, 0.3, 3.2, 4.8, 1.6, NAVY_BG, NAVY)
    ...
    # Part II
    rounded_box(ax, 5.7, 3.2, 5.0, 1.6, GREEN_BG, GREEN)
    ...
    # 부록
    rounded_box(ax, 2.5, 0.8, 6.0, 1.6, AMBER_BG, AMBER)
...
if __name__ == '__main__':
    print("다이어그램 생성 시작 (Beamer Madrid 색상)...")
    funcs = [
